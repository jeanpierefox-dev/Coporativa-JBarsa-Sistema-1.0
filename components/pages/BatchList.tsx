...
const BatchList: React.FC = () => {
  ...
  const refresh = () => {
      ...
      if (user?.role === UserRole.ADMIN) {
          ...
          setBatches(all.filter(b => b.createdBy === user?.id));
      }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">Gestión de Lotes</h2>
            <p className="text-slate-500 font-medium">Administre sus campañas de producción avícola</p>
        </div>
        {canEdit && (
            <button 
            onClick={() => { setCurrentBatch({}); setShowModal(true); }}
            className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-xl flex items-center transition-colors shadow-lg shadow-emerald-200 font-bold"
            >
